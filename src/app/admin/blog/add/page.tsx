...
export default function AddBlogPage() {
  ...
  const handleSubmit = async (status: 'published' | 'draft') => {
    ...
    try {
      ...
      const res = await createAdminBlogPostApi({ ...formData, status });
      if (res.ok) {
        toast.success(status === 'published' ? "Article Deployed!" : "Draft Saved");
        router.push("/admin/blog");
      }
    } catch (err) {
      toast.error("Tactical Failure: Could not save post");
    } finally {
    ...
    <div className="max-w-6xl mx-auto pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black uppercase italic tracking-tighter text-slate-900">Add New Story</h1>
            <p className="text-slate-500 text-sm font-medium tracking-tight">Drafting tactical gear strategy and stories</p>
          </div>
        </div>
