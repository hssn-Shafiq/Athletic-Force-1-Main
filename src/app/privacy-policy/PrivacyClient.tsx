...
import { ShieldCheck, Lock, Eye, FileText, ChevronRight, Gavel, CheckCircle2, AlertCircle } from "lucide-react";
...
export default function PrivacyClient({ initialData }: PrivacyClientProps) {
  const data = initialData || {
    title: "Privacy <span class='text-[#FF5A2A]'>Policy</span>",
    subtitle: "At Athletic Force 1, we value your privacy as much as your performance. This briefing outlines how we handle and protect your operational data.",
    sections: [
      {
    ...
    fullProtocol: `
      <h2 class="text-2xl font-black italic uppercase tracking-tighter text-slate-900 mb-8 border-b border-slate-100 pb-4">Standard Legal Terms</h2>
      <div class="prose prose-slate max-w-none italic font-medium text-slate-600 space-y-6">
         ...
         <h3 class="text-lg font-black text-slate-900 italic uppercase">Cookie Logistics</h3>
         <p>
         ...
         <h3 class="text-lg font-black text-slate-900 italic uppercase">Contact HQ</h3>
         <p>
            If you have questions regarding your data security, contact our security bureau at: <span class="text-[#FF5A2A]">legal@athleticforce1.com</span>
         </p>
      </div>
    `
  };
             ...
             <div key={idx} className="bg-white rounded-[32px] p-8 md:p-12 border border-slate-100 shadow-sm hover:shadow-md transition-shadow group">
                <div className="flex flex-col md:flex-row gap-8">
                   <div className="w-16 h-16 rounded-2xl bg-slate-50 flex items-center justify-center text-[#FF5A2A] group-hover:bg-[#FF5A2A] group-hover:text-white transition-all duration-500 shrink-0 shadow-inner">
                      {ICONS[section.icon] || <FileText className="w-6 h-6" />}
                   </div>
                   <div>
